...
const CONNECTION_NAME = 'emergencyLine';
        ...
        if (turnContext.activity.type === ActivityTypes.Message) {
            ...
            if (validCommands.includes(text)) {
                ...
                if (!turnContext.responded) {
                    let botAdapter = turnContext.adapter;
                    // This instructs the bot to ask for tokens from the emulator rather than the Azure Bot Service.
                    // botAdapter.emulateOAuthCards(turnContext, true);
            ...
            for (let index = 0; index < members.length; index++) {
                ...
                if (member.id !== turnContext.activity.recipient.id) {
                    const welcomeMessage = `Welcome to EmergencyBot ${member.name}. ` + HELP_TEXT;
